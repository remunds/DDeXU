...
def load_cifar100_c_test(test_ds):
    corruptions = [
        "brightness",
        "contrast",
        "defocus_blur",
    ...
    for corr_idx, c in enumerate(corruptions):
        ...
        data = np.load(f"{cifar100_c_path_complete}/{c}.npy")
...
def start_cifar100_calib_run(run_name, batch_sizes, model_params, train_params, trial):
    with mlflow.start_run(run_name=run_name):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        mlflow.log_param("device", device)

        ckpt_dir = f"/data_docker/ckpts/cifar100-c_calib/{run_name}/"
        ...
        print("Eval OOD cifar100-c")
        cifar100_c_ll = model.eval_ll(cifar100_c_dl, device, return_all=True)
        cifar100_c_ll_marg = model.eval_ll_marg(cifar100_c_ll, device, return_all=True)
        mlflow.log_metric("cifar100_c_ll_marg", cifar100_c_ll_marg.mean().item())
        cifar100_c_entropy = model.eval_entropy(cifar100_c_ll, device, return_all=True)
        mlflow.log_metric("cifar100_c_entropy", torch.mean(cifar100_c_entropy).item())

        (_, _, _), (_, _, _), auroc, auprc = model.eval_ood(
            orig_test_pred_entropy, cifar100_c_entropy, device
        )
        mlflow.log_metric("auroc_cifar100_c_entropy", auroc)
        mlflow.log_metric("auprc_cifar100_c_entropy", auprc)

        (_, _, _), (_, _, _), auroc, auprc = model.eval_ood(
            orig_test_ll_marg, cifar100_c_ll_marg, device, confidence=True
        )
        mlflow.log_metric("auroc_cifar100_c_ll_marg", auroc)
        mlflow.log_metric("auprc_cifar100_c_ll_marg", auprc)

        print("Done OOD cifar100-c")
