...
def start_mnist_calib_run_gmm(run_name, batch_sizes, model_params, train_params, trial):
    with mlflow.start_run(run_name=run_name):
        ...
        for s in tqdm(severity_indices):
            ...
            model.gmm_active = True
            gmm_acc = model.eval_acc(test_dl_rot, device)
            ...
            model.gmm_active = True
            gmm_acc = model.eval_acc(test_dl_cutoff, device)
            ...
            severity = get_severity(test_ds, s - 1)
            gmm_acc = model.eval_acc(test_dl_noise, device)
            model.gmm_active = False
        ...
        for m in ["rotation", "cutoff", "noise"]:
            ...
            ax.plot(gmm_acc, label="gmm acc", color="orange")
